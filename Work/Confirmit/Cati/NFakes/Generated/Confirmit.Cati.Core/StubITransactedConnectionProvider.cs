...
{
    public class StubITransactedConnectionProvider : ITransactedConnectionProvider 
    {
        private ITransactedConnectionProvider _inner;

        public StubITransactedConnectionProvider()
        {
            _inner = null;
        }

        public ITransactedConnectionProvider Inner
        {
        ...
        public delegate SqlTransaction BeginTransactionStringDelegate(string transactionName);
        public BeginTransactionStringDelegate BeginTransactionString;

        SqlTransaction ITransactedConnectionProvider.BeginTransaction(string transactionName)
        {


            if (BeginTransactionString != null)
            {
                return BeginTransactionString(transactionName);
            } else if (_inner != null)
            {
                return ((ITransactedConnectionProvider)_inner).BeginTransaction(transactionName);
            }
