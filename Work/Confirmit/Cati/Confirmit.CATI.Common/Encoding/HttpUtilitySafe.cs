...
namespace Confirmit.CATI.Common.Encoding
{
    public sealed class HttpUtilitySafe
    {        
        /// <devdoc> 
        ///    <para>
        ...
        public static void HtmlDecode(string s, TextWriter output)
        {
            ...
            for (int i = 0; i < l; i++)
            {
                ...
                if (ch == '&')
                {
                    ...
                    int index = s.IndexOfAny(EntityEndingChars, i + 1);
                    if (index > 0 && s[index] == ';')
                    {
        ...
        public static string HtmlEncode(string s)
        {
            ...
            var builder = new StringBuilder(s.Length + 5);
            int cch = s.Length;
            int startPos = 0;
            for (; ; )
            {
        ...
        static int IndexOfHtmlEncodingChars(string s, int startPos)
        {
            ...
            for (int i = startPos; i < s.Length; i++, cch--)
            {
                char ch = s[i];
                if (ch <= '>')
                {
