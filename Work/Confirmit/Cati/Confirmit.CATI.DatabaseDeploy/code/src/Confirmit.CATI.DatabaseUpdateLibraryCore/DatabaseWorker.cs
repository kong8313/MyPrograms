using System;
using System.Collections.Generic;
using System.Data;
using System.Data.SqlClient;
using System.Diagnostics;
using System.IO;
using System.Linq;
using Confirmit.CATI.DatabaseUpdateLibraryCore.Interfaces;
using System.Text;

namespace Confirmit.CATI.DatabaseUpdateLibraryCore
{
    public class DatabaseWorker : IDatabaseWorker
    {
        private readonly ILogger _logger;
        private readonly IQueryExecutor _queryExecutor;
        private readonly IConfiguration _configuration;
        private readonly IResources _resources;

        public DatabaseWorker(ILogger logger, IQueryExecutor queryExecutor, IConfiguration configuration, IResources resources)
        {
            _logger = logger;
            _queryExecutor = queryExecutor;
            _configuration = configuration;
            _resources = resources;
        }

        public string CreateConnectionString(string databaseName = "master")
        {
            return _queryExecutor.CreateConnectionString(databaseName);
        }

        private string CreateAdminConnectionString(string databaseName = "master")
        {
            return _queryExecutor.CreateAdminConnectionString(databaseName);
        }

        public bool KillProcesses(string databaseName)
        {
            SqlConnection.ClearAllPools();

            string query = "SELECT DISTINCT request_session_id " + 
                           "FROM master.sys.dm_tran_locks " +
                           $"WHERE resource_type = 'DATABASE' AND resource_database_id = db_id(N'{databaseName}')";

            var dt = _queryExecutor.ExecuteDataTable<DataTable>("master", query);

            foreach (DataRow row in dt.Rows)
            {
                try
                {
                    _queryExecutor.ExecuteNonQuery("master", $"KILL {row[0]}", true);
                }
                catch (Exception ex)
                {
                    _logger.WriteLog(TraceEventType.Warning, "An error during an execution of KillAllProcesses action:\r\n" + ex);
                    return false;
                }
            }

            return true;
        }

        public bool IsDatabaseExists(string databaseName)
        {
            using (var cnScope = new ConnectionScope(CreateConnectionString()))
            {
                string query = $"SELECT count(*) FROM master..sysdatabases where name='{databaseName}'";
                var count = _queryExecutor.ExecuteScalar<int>("master", query, true);

                return count == 1;
            }
        }

        public DatabaseUserAccess GetUserAccess(string databaseName)
        {
            using (var cnScope = new ConnectionScope(CreateConnectionString()))
            {
                string query = $"SELECT user_access_desc FROM sys.databases WHERE name = '{databaseName}'";
                var userAccessStr = _queryExecutor.ExecuteScalar<string>("master", query);

                if (userAccessStr == "SINGLE_USER")
                {
                    return DatabaseUserAccess.Single;
                }

                if (userAccessStr == "MULTI_USER")
                {
                    return DatabaseUserAccess.Multiple;
                }

                return DatabaseUserAccess.Restricted;
            }
        }

        public virtual string ExecuteSqlScript(string sqlQuery, string databaseName)
        {
            var output = new StringBuilder();
            _logger.WriteLog("Execute script for database {0}", databaseName);

            sqlQuery = sqlQuery.Replace("\r\n", "\n");

            if (sqlQuery.ToLower().StartsWith("go\n"))
            {
                sqlQuery = sqlQuery.Substring("go\n".Length);
            }

            if (sqlQuery.ToLower().EndsWith("\ngo"))
            {
                sqlQuery = sqlQuery.Substring(0, sqlQuery.Length - 3);
            }

            string[] queries = sqlQuery
                .Split(new[] { "\nGO\n", "\ngo\n" }, StringSplitOptions.RemoveEmptyEntries)
                .Where(y=>!y.All(char.IsWhiteSpace))
                .Select(x => x.Replace("\n", "\r\n"))
                .ToArray();

            foreach (string query in queries)
            {
                var dt = _queryExecutor.ExecuteDataTable<DataTable>(databaseName, query);

                output.Append(_queryExecutor.OutputOfLastExecution);
                foreach (DataRow row in dt.Rows)
                {
                    var rowInfo = new StringBuilder();
                    foreach (object rowItem in row.ItemArray)
                    {
                        rowInfo.Append(rowItem + " ");
                    }

                    output.Append(rowInfo.Remove(rowInfo.Length - 1, 1) + "\r\n");
                }
            }

            _logger.WriteLog("Script has executed successfully");

            return output.ToString().TrimEnd('\r', '\n');
        }

        public virtual void UpdateRegenerateIsRequiredFlag(string databaseName)
        {
            _logger.WriteLog("Set RegenerateIsRequired flag to 'True' for database {0}", databaseName);

            const string query = "update BvSchedule SET RegenerateIsRequired = 1";
            _queryExecutor.ExecuteDataTable<DataTable>(databaseName, query);

            _logger.WriteLog("RegenerateIsRequired flag has updated successfully");
        }

        public string[] GetAllDatabaseNames()
        {
            string query = "SELECT [name] FROM sys.databases WHERE [state_desc] = 'ONLINE'";
            var dt = _queryExecutor.ExecuteDataTable<DataTable>("master", query);

            return (from DataRow row in dt.Rows select row[0].ToString()).ToArray();
        }

        public void CreateDatabase(string databaseName)
        {
            using (var _ = new ConnectionScope(CreateConnectionString()))
            {
                CreateDatabase(
                    databaseName,
                    _configuration.CatiDatabaseServerDataPath,
                    _configuration.CatiDatabaseServerLogPath,
                    _configuration.AzureSqlServerEdition);
            }
        }

        public void PopulateWithInitialSchemaIfNeeded(string databaseName)
        {
            using (new ConnectionScope(CreateConnectionString(databaseName)))
            {
                var query = "SELECT count(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE='BASE TABLE'";
                int tablesCount = _queryExecutor.ExecuteScalar<int>(databaseName, query);

                if (tablesCount == 0)
                {
                    ExecuteSqlScript(_resources.BaseCreationScript, databaseName);
                }
            }
        }

        public void ApplyNewCompanyUpdates(string databaseName)
        {
            using (new ConnectionScope(CreateConnectionString(databaseName)))
            {
                ExecuteSqlScript(_resources.NewCompanyUpdateScript, databaseName);
            }
        }
        
        public void ConfigureSqlServer()
        {
            using (var _ = new ConnectionScope(CreateAdminConnectionString()))
            {
                //
                // Enable clr for server if needed
                //
                string commandText = "select value_in_use from sys.configurations where name = 'clr enabled'";
                if (_queryExecutor.ExecuteScalar<int>("master", commandText) == 0)
                {
                    commandText = "exec sp_configure 'clr enabled', 1; reconfigure";
                    _queryExecutor.ExecuteNonQuery("master", commandText);
                }

                //
                // Disable clr strict security for server if needed
                //
                commandText = "select value_in_use from sys.configurations where name = 'clr strict security'";
                if (_queryExecutor.ExecuteScalar<int?>("master", commandText) == 1)
                {
                    commandText = @"
                    EXEC sp_configure 'show advanced options', 1 
                    RECONFIGURE;

                    EXEC sp_configure 'clr strict security', 0;
                    RECONFIGURE;";
                    _queryExecutor.ExecuteNonQuery("master", commandText);
                }
            }
        }

        public void OverrideSystemSettingsForContainerEnv()
        {
            var overrides = new Dictionary<string, string>()
            {
                {"Debug.PublishMetadataForExternalWCFServices", "False"},
                {"Debug.PublishMetadataForInternalWCFServices", "False"},
                {"Email.NotificationEmailBCC", "confirmcatiadmins@confirmit.com"},
                {"Server.CreateCompanyDatabasesFromBackup", "False"},
                {"Supervisor.ShowClassicStyleButton", "False"},
                {"Toggle.EnableDesktopConsoleLogin", "False"},
            };

            using (var scope = new ConnectionScope(CreateConnectionString(_configuration.DefaultDatabaseName)))
            {
                foreach (var setting in overrides)
                {
                    _queryExecutor.ExecuteNonQuery(_configuration.DefaultDatabaseName,
                        $"UPDATE BvSystemSettings SET Value = '{setting.Value}' WHERE SystemName = '{setting.Key}'");
                }
            }
        }

        private void CreateDatabase(string databaseName, string mdfPath, string ldfPath, string azureSqlServerEdition)
        {
            azureSqlServerEdition = !string.IsNullOrEmpty(azureSqlServerEdition) ? $"({azureSqlServerEdition})" : ""; 
            if (string.IsNullOrEmpty(mdfPath))
            {
                _queryExecutor.ExecuteNonQuery("master", $"CREATE DATABASE {databaseName} {azureSqlServerEdition}");
            }
            else
            {
                string mdfFile = Path.Combine(mdfPath, databaseName + ".mdf");
                string ldfFile = Path.Combine(ldfPath, databaseName + "_log.ldf");

                _logger.WriteLog("mdfFile={0}\r\nldfFile={1}\r\ndatabase name={2}", mdfFile, ldfFile, databaseName);

                string sqlQuery = string.Format(
                    @"CREATE DATABASE [{0}] {3} ON ( NAME = {0}, FILENAME = '{1}' ) 
					        LOG ON ( NAME = {0}_log, FILENAME = '{2}' )",
                    databaseName,
                    mdfFile,
                    ldfFile,
                    azureSqlServerEdition);

                _queryExecutor.ExecuteNonQuery("master", sqlQuery);
            }
        }
    }
}