ALTER TABLE [dbo].[BvCallCenter]
    ADD [DialerId] INT CONSTRAINT [DF_BvCallCenter_DialerId] DEFAULT (0) NOT NULL;
...
AS
	...
	BEGIN
		...
		RETURN 0
	END
	INSERT INTO BvCallCenter( Name, Description, LocalTimezoneId, DialerId ) VALUES( @Name, @Description, @LocalTimezoneId, @DialerId )
	RETURN SCOPE_IDENTITY()

GO

ALTER PROCEDURE [dbo].[BvSpCallCenter_Update]
	@CallCenterID INT,
	@Name NVARCHAR(MAX),
	@Description NVARCHAR(MAX),
	@LocalTimezoneId INT,
...
ALTER  PROCEDURE [dbo].[BvSpSurvey_GetCountOfLoggedPerson]
        @SurveyId INT,
        @CallCenterId INT,
		@TaskChoiceMode INT
AS
SET NOCOUNT ON
	SELECT COUNT(*) FROM BvTasks t
		...
		WHERE (t.SurveySID = @SurveyId OR @SurveyId = 0 ) AND t.CallCenterID = @CallCenterId AND p.ManualSelection = @TaskChoiceMode
RETURN (0)

GO

CREATE NONCLUSTERED INDEX [IX_BvTask_DialerId] ON [dbo].[BvTasks] 
(
	[DialerId] ASC
)
