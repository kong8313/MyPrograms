...
@SurveySID INT, @SelectInterviewsQuery NVARCHAR (MAX), @ExcludedITSes BvIntArrayType READONLY
AS
...
DECLARE @Query NVARCHAR(MAX) ='
         SELECT
             ...
             (SELECT count( * ) FROM BvInterview i LEFT JOIN @ExcludedITSes s ON i.TransientState = s.Value
              WHERE SurveySID = @SurveySID AND s.Value IS NULL) ''sample_size''
	 ...
	 EXEC sp_executesql  @Query, N'@SurveySID INT, @ExcludedITSes BvIntArrayType READONLY',
     @SurveySID = @SurveySID, @ExcludedITSes = @ExcludedITSes
GO
