...
(
 SELECT
	...
	LEFT JOIN [BvReplicatedData_' + CAST(@SurveyId AS nvarchar(max)) + '] r
		ON ' + @Join + '
	LEFT JOIN BvHistory h
		ON h.SurveyId =' + CAST(@SurveyId AS nvarchar(max)) + ' AND r.respid = h.InterviewId and h.FiredTime between ''' + CONVERT(NVARCHAR(MAX), @StartDate, 112) + ''' AND ''' + CONVERT(NVARCHAR(MAX), DATEADD(dd, 1, @TargetDate), 112) + '''
