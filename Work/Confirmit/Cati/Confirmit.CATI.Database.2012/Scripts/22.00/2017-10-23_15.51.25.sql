...
AS  
	;WITH CustomITSes as (
		SELECT 
			SurveySID,
			SUM( CASE WHEN ITS = @CustomITS1 THEN Cnt ELSE 0 END ) as CustomITS1_Cnt,
			MAX( CASE WHEN ITS = @CustomITS1 THEN AlertStatus ELSE 0 END ) as CustomITS1_Alert,
			SUM( CASE WHEN ITS = @CustomITS2 THEN Cnt ELSE 0 END ) as CustomITS2_Cnt,
			MAX( CASE WHEN ITS = @CustomITS2 THEN AlertStatus ELSE 0 END ) as CustomITS2_Alert,
			SUM( CASE WHEN ITS = @CustomITS3 THEN Cnt ELSE 0 END ) as CustomITS3_Cnt,
			MAX( CASE WHEN ITS = @CustomITS3 THEN AlertStatus ELSE 0 END ) as CustomITS3_Alert,
			SUM( CASE WHEN ITS = @CustomITS4 THEN Cnt ELSE 0 END ) as CustomITS4_Cnt,
			MAX( CASE WHEN ITS = @CustomITS4 THEN AlertStatus ELSE 0 END ) as CustomITS4_Alert,
			SUM( CASE WHEN ITS = @CustomITS5 THEN Cnt ELSE 0 END ) as CustomITS5_Cnt,
