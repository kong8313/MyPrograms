PRINT N'Creating [dbo].[BvSpReportSampleStatusSummaryForDatesRange]...';
...
WHERE h.SurveyId = @SurveySID AND FiredTime BETWEEN @StartDate AND @EndDate
...
PRINT N'Updating [dbo].[BvSpGetCallsSentToDialerDistribution]...';
GO
...
SELECT  @total = SUM(CallsCount) FROM BvCallsSentToDialer WHERE [Time] BETWEEN @tableStartTime AND @tableEndTime
RETURN @total
GO

PRINT N'Creating [dbo].[BvSpGetDialerCallsBreakdown]...';
