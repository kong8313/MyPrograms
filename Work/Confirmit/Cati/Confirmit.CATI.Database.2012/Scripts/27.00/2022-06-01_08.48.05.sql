		...
		BEGIN
			...
			BEGIN
				...
				SET TimeInShift = ( CASE WHEN @TimeToCall = @TimeToCallNow THEN @TimeToCallNow
									...
									END),
					Priority = @Priority,
					CallState = CASE WHEN ( NOT (qcell.CellID IS NOT NULL) OR #InterviewTimeZoneTable.FcdAction = 1) THEN  2 ELSE 1 END,
				...
				LEFT JOIN BvInterviewQuotaCell AS icell
					ON icell.SurveyId = @SurveySID AND icell.InterviewId = #InterviewTimeZoneTable.ID
	...
	BEGIN
		...
		BEGIN
			...
			N'INSERT INTO BvSvySchedule
				...
				LEFT JOIN BvSurveyQuotaCell AS qcell
					ON icell.SurveyID = qcell.SurveyID AND icell.QuotaID = qcell.QuotaID and icell.CellID = qcell.CellID AND qcell.IsOpen = 0
				WHERE ( NOT (qcell.CellID IS NOT NULL) OR #InterviewTimeZoneTable.FcdAction = 1) AND
...
BEGIN
	...
	BEGIN
		...
		SET @Query = 'UPDATE BvSvySchedule SET CallState = CASE WHEN (qcell.CellID IS NOT NULL) THEN 1 ELSE 2 END
		FROM BvTransferArrays ta
		LEFT JOIN BvInterviewQuotaCell AS icell
        ON icell.SurveyID = @SurveySID AND icell.InterviewId = ta.ItemID 
        LEFT JOIN BvSurveyQuotaCell AS qcell
        ON icell.SurveyID = qcell.SurveyID AND icell.QuotaID = qcell.QuotaID and icell.CellID = qcell.CellID AND qcell.IsOpen = 0
		WHERE	BvSvySchedule.SurveySID = @SurveySID AND
