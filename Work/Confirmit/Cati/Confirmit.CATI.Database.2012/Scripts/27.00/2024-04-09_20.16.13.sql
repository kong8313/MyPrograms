	...
	BEGIN
		;WITH Logins AS (
			select lg.ObjectSID as sid, lg.SurveySID, count(*) as cnt
				...
				WHERE t.DialerId = @DialerId AND lg.DialTypeId = 0 AND t.BreakTypeId IS NULL
	...
	BEGIN
		;WITH Logins AS (
			select lg.ObjectSID as sid, lg.SurveySID, count(*) as cnt
				...
				INNER JOIN BvTasks t 
					ON lg.PersonSID = t.PersonSID
				WHERE lg.DialTypeId = 0 AND t.BreakTypeId IS NULL
				group by lg.ObjectSID, lg.SurveySID
		)
		INSERT INTO @Groups
		SELECT c.sid, count(*)
...
SET NOCOUNT ON
	...
	(
		...
		WHERE g.PersonSid = g.ObjectSID AND t.DialerId = @DialerId and g.DialTypeId = 0 AND t.BreakTypeId IS NULL
	)
