...
CREATE PROCEDURE [dbo].[BvSpPerson_SetAutomaticSurvey]
    @PersonId INT,
    @AutoSurveyId INT
AS
...
CREATE TABLE #result( DialerId INT, InterviewState INT, SurveySID INT)
...
UPDATE BvTasks 
    ...
    WHERE PersonSID = @PersonId AND SurveySID > 0 AND StatusLogout NOT IN (0, 1, 4 ) -- LoginState.LOGGING_IN || LoginState.LOGGING_OUT || LoginState.NOT_LOGGED_IN
        ...
        (BvTasks.LoggedInToDialerState = 0/*NOT_LOGGED_IN*/ AND DialMode = 1 /*Manual*/)))
