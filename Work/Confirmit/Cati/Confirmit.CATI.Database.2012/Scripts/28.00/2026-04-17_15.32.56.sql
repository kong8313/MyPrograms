GO
PRINT N'Altering Procedure [dbo].[BvSpGetOpenedSurveys]...';
...
SET NOCOUNT ON
    ...
    SELECT @CallCenterId = CallCenterID, @DialTypeId = DialTypeId FROM BvPerson WHERE SID = @PersonSID  
    ...
    (
        ...
        WHERE s.State = 1
          -- If interviewer DialType is Assisted (2), only show surveys with Manual (1) or Predictive (4) dialing mode  
          AND (@DialTypeId <> 2 OR s.DialMode IN (1 /*Manual*/, 4 /*Predictive*/))  
    )
