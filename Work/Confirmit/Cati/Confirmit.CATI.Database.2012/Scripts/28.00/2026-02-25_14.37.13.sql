GO
PRINT N'Altering Procedure [dbo].[BvSpGetCachedCallsForPredictiveSurveyAssignedToSurveyOnly]...';
...
RETURN (@@ROWCOUNT)
GO
PRINT N'Altering Procedure [dbo].[BvSpGetCachedCallsForPredictiveSurveyByPersonGroup]...';
...
SET NOCOUNT ON
	...
	(
		SELECT TOP(@Count) c.*, a.ShiftPriority
        FROM BvActiveShiftTypeZone a
		CROSS APPLY [GetPriorityCallByExplicitSidAndShiftTypeId](@DialType, @groupID, a.Id, @SurveySID, @SuitableTimeForCalls, @Count) c
		WHERE a.Surveyid = @SurveySID
		ORDER BY Priority DESC,
				 a.ShiftPriority DESC,
                 TimeInShift,
...
RETURN (@@ROWCOUNT)
GO
PRINT N'Altering Procedure [dbo].[BvSpGetCachedCallsForPredictiveSurveyBySurvey]...';
...
RETURN (@@ROWCOUNT)
GO
PRINT N'Altering Procedure [dbo].[BvSpGetCachedCallsForPredictiveSurveyExplicitlyAssigned]...';
