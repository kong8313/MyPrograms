GO
PRINT N'Altering Procedure [dbo].[BvSpGetListSurveyTasks]...';
...
AS
   ...
   (SELECT t.InterviewID, 
          ...
          t.LastKeepAliveTime,
          (CASE WHEN LastKeepAliveTime IS NULL AND IsWebConsole = 0
				THEN 2
				WHEN (LastKeepAliveTime IS NULL AND IsWebConsole = 1) OR p.Type = 1
				THEN 0
				ELSE lkat.val
   ...
   INNER JOIN BvFnPerson_Get(@CallCenterID) p ON (t.PersonSID = p.SID)
   ...
   LEFT JOIN BvCallCenter cc ON cc.ID = tsk.CallCenterID
GO
