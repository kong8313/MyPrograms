...
END
 ...
 SELECT
  ...
  FROM #Surveys s 
  LEFT JOIN BvHistory h ON s.SurveyId = h.SurveyId AND
                           h.FiredTime >= @StartDateTime AND
                           h.FiredTime <= @EndDateTime AND
...
GO
PRINT N'Altering [dbo].[BvSpSurveyProductivityReportCati]...';


GO
ALTER PROCEDURE [dbo].[BvSpSurveyProductivityReportCati]
	@SurveySids NVARCHAR(MAX), 
    @PersonSIDs NVARCHAR (MAX), 
    @ITS NVARCHAR (MAX), 
    @StartDate DATETIME, 
...
AS 
 ...
 BEGIN
    exec BvSpSurveyOverviewReportForAllPersons @SurveySids, @CompletedItses, @UseDialer, @HideEmpty, @StartDateTime, @EndDateTime, @SurveyDataFilter
	return
 END
