...
using ConfirmitDialerInterface32 = CodiV32::ConfirmitDialerInterface;

namespace Confirmit.CATI.Telephony.DialerLibrary
{
    public class CodiVersion32CoreProxy : ICodiVersionCoreProxy
    {
        private readonly IChannelFactoryWrapper<IDialerService32> _dialerChannel;

        public CodiVersion32CoreProxy(IChannelFactoryWrapper<IDialerService32> dialerChannel)
        {
            ...
            bool recordWholeInterview, string campaignParametersXml)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.StartCampaign(
                    ...
                    campaignId,
                    campaignName,
                    (ConfirmitDialerInterface32.DialingMode)dialingMode,
            ...
            IEnumerable<KeyValuePair<string, string>> agentAttributes)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.Login(
                    ...
                    isPredictive,
                    (ConfirmitDialerInterface32.ResourceBindingType)resourceBindingType,
                    agentAttributes));
        }
            ...
            List<CallInfo> callList, int callAgingTimeout)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.SendNumbers(
                    ...
                    dialerId,
                    campaignId,
                    (ConfirmitDialerInterface32.DialingMode)campaignDialingMode,
                    CallList32(callList),
                    callAgingTimeout));
        }
            ...
            InterviewStatus interviewStatus, bool makeAgentReady, string breakName, int interviewId, long callId)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.CompleteCall(
                    ...
                    campaignId,
                    agentId,
                    InterviewStatus32(interviewStatus),
                    makeAgentReady));
        }
            ...
            StopRecordingMode stopRecordingMode)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.StopRecording(
                    ...
                    interviewId,
                    callId,
                    (ConfirmitDialerInterface32.StopRecordingMode)stopRecordingMode));
        }
        ...
        {
            IEnumerable<ConfirmitDialerInterface32.TrunkLineStateAndAlarms> outTrunkLineStatesAndAlarms = null;
        ...
        private ConfirmitDialerInterface32.InterviewStatus InterviewStatus32(InterviewStatus interviewStatus)
        {
            return new ConfirmitDialerInterface32.InterviewStatus { Code = interviewStatus.Code, Name = interviewStatus.Name };
        }

        private List<ConfirmitDialerInterface32.CallInfo> CallList32(IEnumerable<CallInfo> callList)
        {
            return new List<ConfirmitDialerInterface32.CallInfo>(
                callList.Select(x => new ConfirmitDialerInterface32.CallInfo(
                    x.agentId,
                    x.interviewId,
                    x.callId,
                    x.agentGroupId,
                    x.phoneNumber,
                    x.timeToCall,
                    (ConfirmitDialerInterface32.DialingMode)x.diallingMode,
                    x.wasAbandoned,
