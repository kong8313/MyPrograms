...
using DialingMode35 = CodiV35::ConfirmitDialerInterface.DialingMode;
using AudioMessageType35 = CodiV35::ConfirmitDialerInterface.AudioMessageType;
using AudioSourceType35 = CodiV35::ConfirmitDialerInterface.AudioSourceType;
using AudioMessageDescriptor35 = CodiV35::ConfirmitDialerInterface.AudioMessageDescriptor;

using ConfirmitDialerInterface35 = CodiV35::ConfirmitDialerInterface;

namespace Confirmit.CATI.Telephony.DialerLibrary
{
    public class CodiVersion35CoreProxy : ICodiVersionCoreProxy
    {
        private readonly IChannelFactoryWrapper<IDialerService35> _dialerChannel;

        public CodiVersion35CoreProxy(IChannelFactoryWrapper<IDialerService35> dialerChannel)
        {
        ...
        public DialerErrorCode Login(int companyId, int dialerId, long campaignId, int agentId, string agentName, AgentType agentType, string agentConnectionString, bool isPredictive, ResourceBindingType resourceBindingType, IEnumerable<KeyValuePair<string, string>> agentAttributes)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.Login(
                    ...
                    agentId,
                    agentName,
                    (ConfirmitDialerInterface35.AgentType)agentType,
                    agentConnectionString,
                    isPredictive,
                    (ConfirmitDialerInterface35.ResourceBindingType)resourceBindingType,
                    agentAttributes));
        }
            ...
            List<CallInfo> callList, int callAgingTimeout)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.SendNumbers(
                    ...
                    dialerId,
                    campaignId,
                    (DialingMode35)campaignDialingMode,
                    ToCallList35(callList),
                    callAgingTimeout));
        }
            ...
            InterviewStatus interviewStatus, bool makeAgentReady, string breakName, int interviewId, long callId)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.CompleteCall(
                    ...
                    campaignId,
                    agentId,
                    ToInterviewStatus35(interviewStatus),
                    makeAgentReady));
        }
            ...
            InterviewStatus currentInterviewStatus, long nextCampaignId, int nextInterviewId, long nextCallId)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.SetNextInterview(
                    ...
                    currentCampaignId,
                    agentId,
                    ToInterviewStatus35(currentInterviewStatus),
                    nextCampaignId,
                    nextInterviewId,
                    nextCallId));
        }
            ...
            StopRecordingMode stopRecordingMode)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.StopRecording(
                    ...
                    interviewId,
                    callId,
                    (ConfirmitDialerInterface35.StopRecordingMode)stopRecordingMode));
        }
        ...
        {
            IEnumerable<ConfirmitDialerInterface35.TrunkLineStateAndAlarms> outTrunkLineStatesAndAlarms = null;
        ...
        {
            var inboundDdiNumbersV35 = inboundDdiNumbers.Select(
                x => new ConfirmitDialerInterface35.InboundDdiNumber
                {
                    Number = x.Number,
                    AudioMessages = x.AudioMessages
                        .Where(a => Enum.IsDefined(typeof(AudioMessageType35), (int)a.Key))
                        .Select(y => new KeyValuePair<AudioMessageType35, AudioMessageDescriptor35>(
                            (AudioMessageType35)y.Key,
                            ToAudioMessageDescriptor35(y.Value)))
                        .ToArray()
                }).ToArray();
        ...
        public DialerErrorCode DropInboundCall(int companyId, int dialerId, string inboundCallId, AudioMessageDescriptor audioMessageDescriptor)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.DropInboundCall(
                    ...
                    dialerId,
                    inboundCallId,
                    ToAudioMessageDescriptor35(audioMessageDescriptor)));
        }
        ...
        private ConfirmitDialerInterface35.InterviewStatus ToInterviewStatus35(InterviewStatus interviewStatus)
        {
            return new ConfirmitDialerInterface35.InterviewStatus { Code = interviewStatus.Code, Name = interviewStatus.Name };
        }

        private ConfirmitDialerInterface35.CallInfo ToCallInfo35(CallInfo callInfo)
        {
            return new ConfirmitDialerInterface35.CallInfo(
                callInfo.agentId,
                ...
                callInfo.timeToCall,
                (DialingMode35)callInfo.diallingMode,
        ...
        private List<ConfirmitDialerInterface35.CallInfo> ToCallList35(IEnumerable<CallInfo> callList)
        {
            return new List<ConfirmitDialerInterface35.CallInfo>(
                callList.Select(ToCallInfo35)
            );
        }

        private AudioMessageDescriptor35 ToAudioMessageDescriptor35(AudioMessageDescriptor audioMessageDescriptor)
        {
            ...
            return new AudioMessageDescriptor35
            {
                Type = (AudioSourceType35)audioMessageDescriptor.Type,
