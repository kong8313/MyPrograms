...
using ConfirmitDialerInterface33 = CodiV33::ConfirmitDialerInterface;

namespace Confirmit.CATI.Telephony.DialerLibrary
{
    public class CodiVersion33CoreProxy : ICodiVersionCoreProxy
    {
        private readonly IChannelFactoryWrapper<IDialerService33> _dialerChannel;

        public CodiVersion33CoreProxy(IChannelFactoryWrapper<IDialerService33> dialerChannel)
        {
            ...
            bool recordWholeInterview, string campaignParametersXml)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.StartCampaign(
                    ...
                    campaignId,
                    campaignName,
                    (ConfirmitDialerInterface33.DialingMode)dialingMode,
            ...
            IEnumerable<KeyValuePair<string, string>> agentAttributes)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.Login(
                    ...
                    isPredictive,
                    (ConfirmitDialerInterface33.ResourceBindingType)resourceBindingType,
                    agentAttributes));
        }
            ...
            List<CallInfo> callList, int callAgingTimeout)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.SendNumbers(
                    ...
                    dialerId,
                    campaignId,
                    (ConfirmitDialerInterface33.DialingMode)campaignDialingMode,
                    CallList33(callList),
                    callAgingTimeout));
        }
            ...
            InterviewStatus interviewStatus, bool makeAgentReady, string breakName, int interviewId, long callId)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.CompleteCall(
                    ...
                    campaignId,
                    agentId,
                    InterviewStatus33(interviewStatus),
                    makeAgentReady));
        }
            ...
            StopRecordingMode stopRecordingMode)
        {
            return (DialerErrorCode)_dialerChannel.Execute(
                x => x.StopRecording(
                    ...
                    interviewId,
                    callId,
                    (ConfirmitDialerInterface33.StopRecordingMode)stopRecordingMode));
        }
        ...
        {
            IEnumerable<ConfirmitDialerInterface33.TrunkLineStateAndAlarms> outTrunkLineStatesAndAlarms = null;
        ...
        private ConfirmitDialerInterface33.InterviewStatus InterviewStatus33(InterviewStatus interviewStatus)
        {
            return new ConfirmitDialerInterface33.InterviewStatus { Code = interviewStatus.Code, Name = interviewStatus.Name };
        }

        private List<ConfirmitDialerInterface33.CallInfo> CallList33(IEnumerable<CallInfo> callList)
        {
            return new List<ConfirmitDialerInterface33.CallInfo>(
                callList.Select(x => new ConfirmitDialerInterface33.CallInfo(
                    x.agentId,
                    x.interviewId,
                    x.callId,
                    x.agentGroupId,
                    x.phoneNumber,
                    x.timeToCall,
                    (ConfirmitDialerInterface33.DialingMode)x.diallingMode,
                    x.wasAbandoned,
