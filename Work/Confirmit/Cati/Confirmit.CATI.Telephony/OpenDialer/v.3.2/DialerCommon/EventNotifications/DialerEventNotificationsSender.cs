...
namespace Confirmit.CATI.Telephony.DialerCommon.EventNotifications
{
    ...
    public class DialerEventNotificationsSender : IDisposable
    {
        ...
        public DialerEventNotificationsSender(ICommonLogger logger, int dialerId, int companyId)
        {
            ...
            _lowPrioritydialerEventsHandlerServiceClient = new DialerEventsServiceClient(companyId, dialerId, catiCommonLogger);
            _highPrioritydialerEventsHandlerServiceClient = new DialerEventsServiceClient(companyId, dialerId, catiCommonLogger);
        }
        ...
        private bool SendEventNotificationSynchronously(DialerEvent dialerEvent, int currentRetry, int maxRetryCount)
        {
            try
            {
                ...
                _logger.Verbose(
                    ...
                    "Sending({0}/{1}): {2} /// dialerId={3}",
                    currentRetry, maxRetryCount, dialerEvent, _dialerId);
                ...
                _logger.Info(
                    "DialerEventNotificationsSender",
                    "Sent (Duration: {0}): {1} /// dialerId={2}",
                    timer.ElapsedMilliseconds, dialerEvent, _dialerId);
            ...
            catch (Exception ex)
            {
                _logger.Error(
                    ...
                    "{0} /// dialerEvent={1}, dialerId={2}", dialerEvent, _dialerId, ex.ToString());
