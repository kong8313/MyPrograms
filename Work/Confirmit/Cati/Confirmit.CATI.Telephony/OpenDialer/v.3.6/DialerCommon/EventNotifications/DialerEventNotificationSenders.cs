...
namespace Confirmit.CATI.Telephony.DialerCommon.EventNotifications
{
    ...
    {
        private readonly Dictionary<int, IDialerEventNotificationsSender> _companyIdToSenderDictionary =
            new Dictionary<int, IDialerEventNotificationsSender>();

        private readonly INotificationsSenderInitializer _notificationsSenderInitializer;

        public DialerEventNotificationSenders(INotificationsSenderInitializer notificationsSenderInitializer)
        {
            _notificationsSenderInitializer = notificationsSenderInitializer;
        }

        public IDialerEventNotificationsSender GetSender(int companyId, int dialerId)
        {
            if (companyId == 0)
            {
            ...
            IDialerEventNotificationsSender notificationsSender;
