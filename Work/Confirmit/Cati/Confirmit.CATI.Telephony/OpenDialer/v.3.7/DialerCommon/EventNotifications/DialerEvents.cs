...
namespace Confirmit.CATI.Telephony.DialerCommon.EventNotifications
{
    ...
    /// Base Event class
    /// </summary>
    public abstract class DialerEvent : IDialerEvent
    {
    ...
    public class DialerEventNotifyOutcome : DialerEvent
    {
        ...
        public TimeSpan RingTime { get; private set; }
        public string CorrelationId { get; private set; }
        public Dictionary<string, string> CallOutcomeMetadata { get; private set; }

        public DialerEventNotifyOutcome(DialerEventPriority eventPriority, int companyId, long surveyId,
            int agentId, long callId, int outcomeCode,
            string callerId, TimeSpan ringTime, Dictionary<string, string> callOutcomeMetadata, string correlationId)
            : this(eventPriority, companyId, 0, surveyId, agentId, callId, outcomeCode, callerId, ringTime, callOutcomeMetadata, correlationId)
        {
        }

        public DialerEventNotifyOutcome(DialerEventPriority eventPriority, int companyId, int dialerId, long surveyId, int agentId, long callId, int outcomeCode,
            string callerId, TimeSpan ringTime, Dictionary<string, string> callOutcomeMetadata, string correlationId)
            : base(eventPriority, companyId, dialerId, surveyId)
        {
            AgentId = agentId;
    ...
    public class DialerEventNotifyCustomIvrInterviewEnd : DialerEvent
    {
        public int AgentId { get; private set; }
        public long CallId { get; private set; }
        ...
        public DialerEventNotifyCustomIvrInterviewEnd(DialerEventPriority eventPriority, int companyId, long surveyId,
            int agentId, int interviewId, long callId, CallOutcome callOutcome)
            : this(eventPriority, companyId, 0, surveyId, agentId, interviewId, callId, callOutcome)
        {
        }

        public DialerEventNotifyCustomIvrInterviewEnd(DialerEventPriority eventPriority, int companyId, 
            int dialerId, long surveyId, int agentId, int interviewId, long callId, CallOutcome callOutcome)
            : base(eventPriority, companyId, dialerId, surveyId)
        {
            AgentId = agentId;
            CallId = callId;
        ...
        public override void SendEventNotification(DialerEventsServiceClient dialerEventsHandlerServiceClient)
        {
            dialerEventsHandlerServiceClient.NotifyCustomIvrInterviewEnd(
                DialerId,
                CompanyId,
                SurveyId,
                AgentId,
        ...
        public override string ToString()
        {
            return string.Format(
                "{0}, agentId={1}, callId={2}, outcomeCode={3}]",
                base.ToString(), AgentId, CallId, CallOutcome);
        }
    }
    
    public class DialerEventNotifyInboundCall : DialerEvent
    {
        public string DdiNumber { get; private set; }
        public string CliNumber { get; private set; }
        public string InboundCallId { get; private set; }

        public DialerEventNotifyInboundCall(DialerEventPriority eventPriority, int companyId, int dialerId, string ddiNumber, string cliNumber, string inboundCallId)
        ...
        {
            return string.Format(
                "{0}, DdiNumber={1}, CliNumber={2}, InboundCallId={3}]",
                base.ToString(), DdiNumber, CliNumber, InboundCallId);
        }
    }

    public class DialerEventNotifyCallDroppedByRespondent : DialerEvent
    {
        public int AgentId { get; }
        public long CallId { get; }

        public DialerEventNotifyCallDroppedByRespondent(
            DialerEventPriority eventPriority, int companyId, int dialerId, long surveyId, int agentId, long callId)
            : base(eventPriority, companyId, dialerId, surveyId)
        {
            AgentId = agentId;
            CallId = callId;
        ...
        public override void SendEventNotification(DialerEventsServiceClient dialerEventsHandlerServiceClient)
        {
            dialerEventsHandlerServiceClient.NotifyCallDroppedByRespondent(
                DialerId,
                CompanyId,
                SurveyId,
                AgentId,
    ...
    public class DialerEventNotifyInboundCallDroppedByRespondent : DialerEvent
    {
        public string InboundCallId { get; private set; }

        public DialerEventNotifyInboundCallDroppedByRespondent(DialerEventPriority eventPriority, int companyId, int dialerId, string inboundCallId)
            : base(eventPriority, companyId, dialerId, 0)
        {
            InboundCallId = inboundCallId;
        }
        ...
        public override void SendEventNotification(DialerEventsServiceClient dialerEventsHandlerServiceClient)
        {
            dialerEventsHandlerServiceClient.NotifyInboundCallDroppedByRespondent(
                DialerId,
                CompanyId,
                InboundCallId);
        }
    ...
    public class DialerEventNotifyTransferState : DialerEvent
    {
        public string TransferId { get; }
        public TransferState TransferState { get; }

        public DialerEventNotifyTransferState(DialerEventPriority eventPriority, int companyId, int dialerId, string transferId, TransferState transferState)
            : base(eventPriority, companyId, dialerId, 0)
        {
        ...
        public override void SendEventNotification(DialerEventsServiceClient dialerEventsHandlerServiceClient)
        {
            dialerEventsHandlerServiceClient.NotifyTransferState(
                DialerId,
                CompanyId,
                TransferId,
                TransferState);
        ...
        {
            return string.Format(
                "{0}, TransferId={1}, TransferState={2}",
                base.ToString(), TransferId, TransferState);
        }
    }
}
