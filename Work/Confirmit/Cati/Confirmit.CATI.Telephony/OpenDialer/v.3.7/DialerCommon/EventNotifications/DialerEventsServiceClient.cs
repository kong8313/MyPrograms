...
namespace Confirmit.CATI.Telephony.DialerCommon.EventNotifications
{
    ...
    public class DialerEventsServiceClient : IDisposable
    {
        ...
        public DialerEventsServiceClient(int companyId, ILoggerCatiCommon logger)
        {
            ...
            var configuration = string.IsNullOrEmpty(confirmitDefaultHostname)
                ? (IChannelFactoryWrapperConfiguration)new DialerEventsHandlerServiceChannelFactoryWrapperConfiguration(companyId)
                : new DialerEventsHandlerServiceInternalChannelFactoryWrapperConfiguration(companyId);
        ...
        {
            _channelFactoryWrapper.Execute(
                dialerEventsHandler => dialerEventsHandler.NotifyCustomIvrInterviewEnd(
                    dialerId,
                    companyId,
                    campaignId,
                    agentId,
