...
namespace Confirmit.CATI.Telephony.DialerService
{
    ...
    public class DialerService : IDialerService, IDisposable, IDialerEvents
    {
            ...
            INotificationsSenderInitializer notificationsSenderInitializer)
        {
            ...
            if (Settings.Default.UseErrorReporting)
            {
        ...
        public DialerErrorCode Release(int dialerId, int companyId)
        {
            ...
            return Execute(0,
                ...
                argumentsAsString,
                () => DoDialerCall(
                    () => _bridgeToDialer.Release(dialerId, companyId)));
        }

        public DialerFeatures GetFeatures(int companyId, int dialerId)
        {
            var argumentsAsString = $"companyId={companyId}, dialerId={dialerId}";

            var supportedFeatures = Execute(companyId,
                "DialerService.GetFeatures",
                argumentsAsString,
                () => DoDialerCall(
                    () => _bridgeToDialer.GetFeatures(companyId, dialerId)));

            if (supportedFeatures == null)
                return null;

            return new DialerFeatures(supportedFeatures);
        }
        ...
        public DialerErrorCode SendNumberToAgent(int companyId, int dialerId, long campaignId, int agentId, DialingMode dialingMode, int interviewId, long callId, string phoneNumber, bool isRecording, string callerId, Dictionary<string, object> respondentVariables)
        {
            var argumentsAsString = string.Format(
                "companyId={0}, dialerId={1}, campaignId={2}, agentId={3}, dialingMode={4}, interviewId={5}, callId={6}, phoneNumber={7}, isRecording={8}, callerId = {9}, respondentVariables={10}",
                companyId, dialerId, campaignId, agentId, dialingMode, interviewId, callId, phoneNumber, isRecording, callerId, respondentVariables?.Stringify());
            ...
            return Execute(companyId,
                ...
                () => DoDialerCall(
                        ...
                        companyId, dialerId, campaignId, agentId, dialingMode, interviewId, callId, phoneNumber, isRecording, callerId, respondentVariables)));
        }
            ...
            return Execute(companyId,
                ...
                () =>
                {
                    var result = DoDialerCall(
                        () => _bridgeToDialer.Hangup(companyId, dialerId, campaignId, agentId, interviewId, callId));
            ...
            InterviewStatus interviewStatus, bool makeAgentReady, string breakName, int interviewId, long callId)
        {
            var argumentsAsString = string.Format(
                ...
                companyId, dialerId, campaignId, agentId, interviewStatus, makeAgentReady, makeAgentReady ? "NULL" : breakName, interviewId, callId);

            return Execute(companyId,
                "DialerService.CompleteCall",
                argumentsAsString,
                () => DoDialerCall(
                    () => _bridgeToDialer.CompleteCall(companyId, dialerId, campaignId, agentId, interviewStatus, makeAgentReady, breakName, interviewId, callId)));
        }
            ...
            int interviewId, long callId, string respondentSurveyLink)
        {
            ...
            return Execute(companyId,
                "DialerService.StartCustomIvrInterview",
                argumentsAsString,
                () => DoDialerCall(
                    () => _bridgeToDialer.StartCustomIvrInterview(companyId, dialerId, campaignId, agentId, interviewId, callId, respondentSurveyLink)));
        }
        ...
        {
            string argumentsAsString = $"dialerId={dialerId}";
            ...
            Execute(0,
                ...
                () => DoDialerCall(() =>
            {
                _bridgeToDialerRecording.InitializeRecording(dialerId);
        ...
        public IEnumerable<AudioRecordInfo> GetAudioRecords(int companyId, long campaignId, int interviewId, int dialerId)
        {
            ...
            return Execute(0,
                ...
                () => DoDialerCall(() =>
                    {
                        var result = _bridgeToDialerRecording.GetAudioRecords(companyId, campaignId, interviewId, dialerId);
        ...
        public AudioFile GetAudioFile(int companyId, int dialerId, string audioUrl)
        {
            ...
            return Execute(0,
                "DialerService.GetAudioFile",
                argumentsAsString,
                () => DoDialerCall(() =>
                    {
                        var result = _bridgeToDialerRecording.GetAudioFile(companyId, dialerId, audioUrl);

                        Logger.Verbose(
                            "DialerService.GetAudioFile",
                            $"companyId={companyId}, audioUrl={audioUrl}. " + 
                            $"Result: FileName={result?.FileName}, CreationTime={result?.CreationTime}, Content.Count={result?.Content?.Length ?? 0}");
        ...
        {
            var argumentsAsString = string.Format(
                "companyId={0}, interviewCount={1}, dialerId={2}",
                companyId, interviewIdentities.Count(), dialerId);

            return Execute(0,
                "DialerService.GetBulkAudioRecords",
                argumentsAsString,
                () => DoDialerCall(
                    () => _bridgeToDialerRecording.GetBulkAudioRecords(companyId, interviewIdentities, dialerId)));
        }

        public bool[] AreRecordsExists(int companyId, long campaignId, int[] interviewIds, int dialerId)
        {
            var argumentsAsString = string.Format(
                "companyId={0}, campaignId={1}, interviewIds={2}, dialerId={3}",
                companyId, campaignId, string.Join(", ", interviewIds.Select(s => s.ToString())), dialerId);
            ...
            return Execute(0,
                ...
                () => DoDialerCall(() =>
                    {
                        var result = _bridgeToDialerRecording.AreRecordsExists(companyId, campaignId, interviewIds, dialerId);
            ...
            bool isSyncronous = false)
        {
            try
            {
                ...
                if (isSyncronous)
                {
                    notificationSender.SendEventNotificationSynchronously(dialerEvent, TimeSpan.Zero);
                }
                else
                {
        ...
        /// Forsta CATI (asynchronously) waits for this event after calling IDialerCoreApi.Login. 
        /// </summary>
        /// <param name="companyId">Forsta company id</param>
        /// <param name="dialerId">Dialer id</param>
        ...
        /// <param name="callerId">Phone number of the diale</param>
        /// <param name="ringTime">Dialing time</param>
        /// <param name="callOutcomeMetadata">Some additional call info received from dialer</param>
            ...
            string correlationId)
        {
            ...
            Logger.Verbose("DialerService.NotifyOutcome",
                ...
                "interviewId={6}, callId={7}, outcome={8}({9}), " +
                ...
                interviewId, callId, outcome, (int)outcome,
        ...
        public void NotifyCustomIvrInterviewEnd(
            int companyId,
            int dialerId,
            long campaignId,
            int agentId,
            ...
            CallOutcome callOutcome)
        {
            ...
            Logger.Verbose("DialerService.NotifyCustomIvrInterviewEnd",
                "companyId={0}/{1}, dialerId={2}/{3}, campaignId={4}, agentId={5}, " +
                "interviewId={6}, callId={7}, outcome={8}({9})",
                companyId, chosenCompanyId, dialerId, chosenDialerId, campaignId, agentId,
                interviewId, callId, callOutcome, (int)callOutcome);
            
            var dialerEvent = new DialerEventNotifyCustomIvrInterviewEnd(
        ...
        public DialerErrorCode RegisterAgentSoftphone(int companyId, int dialerId, int agentId, string agentName, out string login, out string password, out string host, out string extension, out string frontendUrl)
        {
            ...
            var internalFrontendUrl = "";

            var result = Execute(companyId,
                "DialerService.RegisterAgentSoftphone",
                argumentsAsString,
                () => DoDialerCall(
                    () => _bridgeToDialer.RegisterAgentSoftphone(
                        companyId, dialerId, agentId, agentName, out internalLogin, out internalPassword, out internalHost, out internalExtension, out internalFrontendUrl)));
            
            login = internalLogin;
            password = internalPassword;
            host = internalHost;
            extension = internalExtension;
            frontendUrl = internalFrontendUrl;
            
            Logger.Info("DialerService.RegisterAgentSoftphone",
                $"Returned values: login={login}, password={password}, host={host}, extension={extension}, frontendUrl={frontendUrl}  /// RegisterAgentSoftphoneResult={result}, {argumentsAsString}");
