...
{
    [TestClass]
    public class RestApiCallHistoryTests
    {
        ...
        public async Task RestApi_CallHistoryService_Get()
        {
            short its = 1000;
            ...
            ServiceLocator.Resolve<IHistoryRepository>().Insert(history);
            
            var actual = await _callHistoryService.GetAsync("");
            Assert.AreEqual(1, actual.Count);
            Assert.AreEqual(its, actual[0].ExtendedStatus);
        }
    }
}