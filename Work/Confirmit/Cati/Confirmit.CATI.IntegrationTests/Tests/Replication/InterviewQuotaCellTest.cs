...
{
    [TestClass]
    public class InterviewQuotaCellTest : BaseMockedIntegrationTest
    {
        private IReplicationService _replicationService;

        public override void OnPostTestInitialize()
        {
            _replicationService = ServiceLocator.Resolve<IReplicationService>();
        }

        [TestMethod, Owner(@"FIRM\EgorK")]
        public void CreatingAndUpdatingInterview_DataRreplication_CorrectQuotaCellData()
        {
            ...
            var interviewQuotaCells = BvInterviewQuotaCellAdapter.GetByCondition(query, new[] {
            ...
            _replicationService.RunForceReplication();

            interviewQuotaCells = BvInterviewQuotaCellAdapter.GetByCondition(query, new[] {
                new SqlParameter("SurveyId", survey.Id),
                new SqlParameter("QuotaId", 1)
            });
            ...
            BackendTools.DeleteSurvey(survey.Data.ProjectId);
            
            var surveyQuotaCells = BvSurveyQuotaCellAdapter.GetByCondition(
                query, new SqlParameter("SurveyId", survey.Id), new SqlParameter("QuotaId", 1));
            Assert.AreEqual(0, surveyQuotaCells.Count);
            
            var surveyQuotas = BvSurveyQuotaAdapter.GetByCondition(
                query, new SqlParameter("SurveyId", survey.Id), new SqlParameter("QuotaId", 1));
            Assert.AreEqual(0, surveyQuotas.Count);
