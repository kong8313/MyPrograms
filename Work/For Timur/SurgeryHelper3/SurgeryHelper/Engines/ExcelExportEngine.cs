...
namespace SurgeryHelper.Engines
{
    public class ExcelExportEngine
    {
        ...
        private static object GetKsgNameByCode(object[,] ksgData, string ksgCode)
        {
            for (int i = 0; i < ksgData.GetLength(0); i++)
            {
                if (ksgData[i + 1, 1].ToString() == ksgCode)
                {
                    return ksgData[i + 1, 2];
                }
            }
